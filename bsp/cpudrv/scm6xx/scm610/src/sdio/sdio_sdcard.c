...
//%$#@describe      ****组件描述开始
//component name:"cpu onchip sdio"//CPU的sdio外设驱动
...
//init time:none                //初始化时机，可选值：early，medium，later, pre-main。
                                //表示初始化时间，分别是早期、中期、后期
//dependence:"device file system","heap","cpu onchip dma"//该组件的依赖组件名（可以是none，表示无依赖组件），
                                //选中该组件时，被依赖组件将强制选中，
                                //如果依赖多个组件，则依次列出，用“,”分隔
...
//%$#@configue      ****参数配置开始
#if ( CFG_MODULE_ENABLE_CPU_ONCHIP_SDIO == false )
//#warning  " cpu_onchip_sdio  组件参数未配置，使用默认配置"
//%$#@target = header           //header = 生成头文件,cmdline = 命令行变量，DJYOS自有模块禁用
#define CFG_MODULE_ENABLE_CPU_ONCHIP_SDIO    false //如果勾选了本组件，将由DIDE在project_config.h或命令行中定义为true
//%$#@num,0,100
//%$#@enum,true,false
//%$#@string,1,10
...
u32 SDIO_IRQHandler(ptu32_t IsrNo);
...
SD_Error SD_Init(void)
{
    ...
    GPIO_CfgPinFunc(GPIO_C, (0X1F<<8), GPIO_MODE_AF, GPIO_OTYPE_PP, GPIO_SPEED_50M, GPIO_PUPD_PU); //PC8,9,10,11,12复用功能输出
    GPIO_CfgPinFunc(GPIO_D, (1<<2), GPIO_MODE_AF, GPIO_OTYPE_PP, GPIO_SPEED_50M, GPIO_PUPD_PU);    //PD2复用功能输出
    GPIO_AFSet(GPIO_C, 8, 12);    //PC8,AF12
    GPIO_AFSet(GPIO_C, 9, 12);    //PC9,AF12
    GPIO_AFSet(GPIO_C, 10, 12);   //PC10,AF12
    GPIO_AFSet(GPIO_C, 11, 12);   //PC11,AF12
    GPIO_AFSet(GPIO_C, 12, 12);   //PC12,AF12
    GPIO_AFSet(GPIO_D, 2, 12);    //PD2,AF12
    ...
    {//SDIO中断配置
    Int_Register(CN_INT_LINE_SDIO);
    Int_IsrConnect(CN_INT_LINE_SDIO, SDIO_IRQHandler);
    Int_SettoReal(CN_INT_LINE_SDIO);
    Int_ClearLine(CN_INT_LINE_SDIO);
    Int_RestoreRealLine(CN_INT_LINE_SDIO);
    }
