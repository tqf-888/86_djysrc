...
u32 UART_ISR(ptu32_t port)
{
    ...
    if ((Reg->ctrl1_bit.idleien != RESET) && (usart_flag_get(Reg, USART_IDLEF_FLAG) != RESET))   //总线空闲
    {
        // Reg->DR;
        ch = (u8)usart_data_receive(Reg);
        if (DMA_NONUSE != UartTX_RX_DmaChannel[port][UART_RX_DMA_CH])
        {
...
u32 __UART_DMA_SendStart(u32 port)
{
    ...
    if (DMA_NONUSE == UartAllDmaChannel[UartTX_RX_DmaChannel[port][UART_TX_DMA_CH]])
    {
        return 0;
    }
...
void __UART_SetDmaUsed(u32 port)
{
    ...
    if (DMA_NONUSE != UartTX_RX_DmaChannel[port][UART_RX_DMA_CH])
    {
        ...
        DMA_IntEnable(UartAllDmaChannel[UartTX_RX_DmaChannel[port][UART_RX_DMA_CH]],DMA_INT_TCIE);  //使能接收完成中断
    ...
    if (DMA_NONUSE != UartTX_RX_DmaChannel[port][UART_TX_DMA_CH])
    {
...
void __UART_SetDmaUnUsed(u32 port)
{
    ...
    if (DMA_NONUSE != UartTX_RX_DmaChannel[port][UART_RX_DMA_CH])
    {
        free(pUART_DmaRecvBuf[port]);
        pUART_DmaRecvBuf[port]=NULL;
        usart_dma_receiver_enable(tg_UART_Reg[port], false);
        Int_SaveAsynLine(UartAllDmaChannelInt[UartTX_RX_DmaChannel[port][UART_RX_DMA_CH]]);
    ...
    if (DMA_NONUSE != UartTX_RX_DmaChannel[port][UART_TX_DMA_CH])
    {
        ...
        usart_dma_transmitter_enable(tg_UART_Reg[port], false);
        Int_SaveAsynLine(UartAllDmaChannelInt[UartTX_RX_DmaChannel[port][UART_TX_DMA_CH]]);
...
//    ret = Int_HighAtomStart();
    flag = UartTX_RX_DmaChannel[port][UART_TX_DMA_CH] == DMA_NONUSE ? CN_DMA_UNUSED : CN_DMA_USED;
    __UART_TxIntDisable(flag,port);
    if(flag)
    {
