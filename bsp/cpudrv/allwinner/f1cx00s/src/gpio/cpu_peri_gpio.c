...
{
    u32 config = GPIOx->CFG[GPIO_Pin/8] &(~(0xF<<(GPIO_Pin%8*4)));//清0

//  if(GPIOMode==GPIO_Mode_IN)
//      config &= ~ (0x1<<(GPIO_Pin%8*4));//输入模式
//  else if(GPIOMode==GPIO_Mode_OUT)
//      config |=   (0x1<<(GPIO_Pin%8*4));//输出模式
//  else

    config |=   (GPIOMode<<(GPIO_Pin%8*4));//其它模式
    GPIOx->CFG[GPIO_Pin/8] = config;
    //上下拉
