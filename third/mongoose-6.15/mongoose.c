...
#ifndef MBUF_REALLOC
#define MBUF_REALLOC djyos_realloc//MG_REALLOC
#endif
...
//  /* 释放size空间 */
  if (size == 0)
  {
      if (p) MBUF_FREE(p,CN_TIMEOUT_FOREVER);
  }
  else
  {
//      printf("djyos_realloc: 0x%x, %d!\r\n", p, size);
      ...
      if (p == 0)
      {
          rp = M_Malloc(size, CN_TIMEOUT_FOREVER);
          if (rp==0)
          {
...
//            heap();
      ...
      else
      {
           ...
           if (rp && p)
           {
               old_size = (size_t)M_CheckSize(p);
               old_size = old_size < size ? old_size : size;
               memcpy(rp, p, old_size);
               MBUF_FREE(p,CN_TIMEOUT_FOREVER);
           }
      }
  }
    return rp;
...
#ifndef MBUF_REALLOC
#define MBUF_REALLOC djyos_realloc //realloc
#endif
...
static sock_t mg_open_listening_socket(union socket_address *sa, int type,
  ...
  if ((sock = socket(sa->sa.sa_family, type, proto)) != INVALID_SOCKET &&
      ...
      (type == SOCK_DGRAM || listen(sock, MG_SOMAXCONN) == 0)) {
#if !MG_LWIP
...
static sock_t mg_open_listening_socket(struct mg_connection *nc,
  ...
  if (type != SOCK_DGRAM) {
    if ((r = sl_Listen(sock, MG_SOMAXCONN)) < 0) goto clean;
  }
  mg_set_non_blocking_mode(sock);
clean:
  if (r < 0) {
