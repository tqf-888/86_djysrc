...
struct pci_controller *pci_bus_to_hose(s32 bus)
{
    ...
    debug_printf("pci","pci_bus_to_hose() failed\r\n");
    return NULL;
}
                                 ...
                                 unsigned long flags)
{
    ...
    if (!hose)
    {
        debug_printf("pci","pci_hose_phys_to_bus: invalid hose\r\n");
        return bus_addr;
    }
    ...
    if (ret)
        debug_printf("pci","pci_hose_phys_to_bus: invalid physical address\r\n");
                                 ...
                                 unsigned long flags)
{
    ...
    if (!hose)
    {
        debug_printf("pci","pci_hose_bus_to_phys: invalid hose\r\n");
        return phys_addr;
    }
    ...
    if (ret)
        debug_printf("pci","pci_hose_bus_to_phys: invalid physical address\r\n");
...
//               (u64)mem, command);
    debug_printf("pci","PCI Config: I/O=0x%lx, Memory=0x%llx, Command=0x%lx\r\n", io,
                 (u64)mem, command);
...
s32 pciauto_config_device(struct pci_controller *hose, pci_dev_t dev)
{
    ...
    case PCI_CLASS_BRIDGE_PCI:
        ...
        debug_printf("pci","bridge prebus:%d\r\n",prebus);
        while(1)
        {
            hose->current_busno++;
            BridgeNum = 0;
...
//            pcie_debug("PCI Autoconfig: Found P2P bridge, device %d\r\n", PCI_DEV(dev));
            debug_printf("pci","PCI Autoconfig: Found P2P bridge, device %d\r\n", PCI_DEV(dev));
...
//                               PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev));
                    debug_printf("pci","PCI Scan: Found Bus %d, Device %d, Function %d\r\n",
                            PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev));
...
//                                  pcie_debug("PCI Autoconfig: Skipping legacy mode IDE controller\r\n");
                                  debug_printf("pci","PCI Autoconfig: Skipping legacy mode IDE controller\r\n");
                                  break;
                             }
...
//                                               PCI_DEV(dev));
                             debug_printf("pci","PCI Autoconfig: Found P2CardBus bridge, device %d\r\n",
                                     PCI_DEV(dev));
                                break;
                        case PCI_CLASS_PROCESSOR_POWERPC: /* an agent or end-point */
//                             pcie_debug("PCI AutoConfig: Found PowerPC device\r\n");
                             debug_printf("pci","PCI AutoConfig: Found PowerPC device\r\n");
                        default:
                             pciauto_setup_device(hose, dev, 6, hose->pci_mem,
                                                         hose->pci_prefetch, hose->pci_io);
                             break;
                    ...
                    if (__pci_print_dev(hose, dev))
                    {
                        ...
                        debug_printf("pci","%02x:%02x.%-*x - %04x:%04x - %s\r\n",
        ...
        debug_printf("pci","hose->current_busno :%d\r\n",hose->current_busno);
