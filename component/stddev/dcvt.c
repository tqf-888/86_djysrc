...
static char nextdigit(double *value)
{
  double tmp;
...
static char *print_nan(char *buffer, double value, int precision)
{
  int  i;
...
void renormalize(cvt_info_type *in)
{
...
static void normalize(double value, cvt_info_type *in)
{
  int j;
  int texp=0;
  in->value_neg = 0;
...
int round1(cvt_info_type *in, char *start, char *now, char ch)
{
  double rounder = 5.0;
...
void _cvte(register  cvt_info_type *in)
{
  int buffer_idx  =0;
  int digit = 0;
...
char * _dcvt(char *buffer ,double invalue ,int precision ,int width ,char type ,int dot)
{
  cvt_info_type in;
