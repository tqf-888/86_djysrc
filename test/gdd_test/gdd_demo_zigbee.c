...
static ptu32_t HmiCreate(struct WindowMsg *pMsg)
{
    ...
    Widget_CreateButton("关闭",WS_BORDER|WS_DLGFRAME|BS_SIMPLE|WS_VISIBLE,rc0.left+4,rc0.top+100,56,56,hwnd,ID_CLOSE,NULL,NULL);
    ...
    Widget_CreateButton("LED1",WS_BORDER|BS_HOLD|WS_VISIBLE,x,y+1*24+8,72,20,hwnd,ID_LED1,NULL,NULL);
    Widget_CreateButton("LED2",WS_BORDER|BS_HOLD|WS_VISIBLE,x,y+2*24+8,72,20,hwnd,ID_LED2,NULL,NULL);

    x=rcGroupBox_CTR.left+12;
    y=rcGroupBox_CTR.top+18;
    Widget_CreateButton("组网",WS_BORDER|WS_VISIBLE,x,y+1*24+8,72,20,hwnd,ID_NET_ON,NULL,NULL);
    Widget_CreateButton("断开",WS_BORDER|WS_VISIBLE,x,y+2*24+8,72,20,hwnd,ID_NET_OFF,NULL,NULL);

    Widget_CreateButton("发送",WS_BORDER|BS_SIMPLE|WS_VISIBLE,GDD_RectW(&rc0)-60,rcGroupBox_CTR.top-(24+2),56,24,hwnd,ID_SEND,NULL,NULL);
...
void    GDD_Demo_ZigBee(void)
{
    ...
    GDD_CreateGuiApp("Zigbee", &s_gZigbeeDemoMsgLink, 0,0,-1,0, 0x800, CN_WINBUF_PARENT,
